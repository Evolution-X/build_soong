...
func AutogeneratedRroModuleName(ctx android.EarlyModuleContext, moduleName, partition string) string {
	...
	product := strings.Replace(ctx.Config().Getenv("TARGET_PRODUCT"), "lineage_", "evolution_", 1)
	overrideName := strings.Replace(strings.Replace(strings.Replace(moduleName, "lineageos", "evolution", -1), "Lineage", "Evolution", -1), "lineage", "evolution", -1)
